...
class BBBPTrainer:
    ...
    def run(self, seed):
        ...
        for feature_file in feature_files:
            ...
            # Handle both formats: kernel_{number}.npz and old format like B3DB_cls_ker123_cutoff12.0.npz
            stem = feature_file.stem
            if stem.startswith("kernel_"):
                # New format: kernel_123
                kernel_num = int(stem.split("_")[1])
            else:
                # Old format: try to extract from various patterns
                parts = stem.split("_")
                kernel_num = None
                for part in parts:
